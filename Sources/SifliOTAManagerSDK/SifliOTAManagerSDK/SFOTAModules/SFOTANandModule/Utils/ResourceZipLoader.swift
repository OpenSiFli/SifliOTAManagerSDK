...
class ResourceZipLoader{
    ...
    private static let cacheDirectory = "sifli_ota_nand_res"
    
    static func loadSorted(path:URL) throws -> [NandResFile]  {
        var unzipDirectory:URL
        let isZipFile = isZipFile(rootPath: path)
        if isZipFile {
            unzipDirectory = try reCreateCacheDirectory(directory: cacheDirectory)
            let fileManager = FileManager()
            try fileManager.unzipItem(at: path, to: unzipDirectory)
